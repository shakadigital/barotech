...
export function AttendancePage(state) {
  ...
  function renderRow(l, idx) {
    ...
    // Admin can view employee history
    if (isAdmin) {
      actions += `
        <button class="btn btn-ghost btn-sm" onclick="window.__app.openEmployeeHistoryModal('${l.employee_id}')" title="Lihat Riwayat">
...
/** Buka modal riwayat karyawan (Opsi A) */
export async function openEmployeeHistoryModal(employeeId, state) {
  ...
  const { data: activities } = await supabase
    ...
    .select('*')
    .in('attendance_id', logs.map(l => l.id));

  // Calculate totals
  const verifiedLogs = logs.filter(l => l.status === 'verified');
  const totalGaji = verifiedLogs.reduce((s, l) => s + (l.basic_salary || 0), 0);
  ...
  const totalBersih = totalGaji + totalLembur - totalKasbon;
  ...
  document.getElementById('emp-history-modal')?.remove();

  const modal = document.createElement('div');
  modal.id = 'emp-history-modal';
  modal.className = 'modal-overlay';
  modal.innerHTML = `
    <div class="modal-card" style="max-width:800px;max-height:90vh;overflow-y:auto;">
      <div class="modal-title">
        <i class="fas fa-history"></i> Riwayat Karyawan
        <button onclick="document.getElementById('emp-history-modal').remove()"
          style="margin-left:auto;background:none;border:none;color:var(--text-secondary);cursor:pointer;font-size:1.2rem;">✕</button>
      </div>

      <div class="mb-16" style="background:var(--bg-hover);border-radius:var(--radius);padding:12px 16px;">
        <div class="fw-bold">${esc(emp.full_name)}</div>
        <div class="text-xs text-secondary">${esc(emp.jabatan || 'Karyawan')}</div>
      </div>

      <!-- Ringkasan -->
      <div class="mb-16" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:12px;">
        <div class="text-center">
          <div class="text-xs text-secondary">Total Hari</div>
          <div class="fw-bold">${verifiedLogs.length}</div>
        </div>
        <div class="text-center">
          <div class="text-xs text-secondary">Gaji Pokok</div>
          <div class="fw-bold">${fmtIdr(totalGaji)}</div>
        </div>
        <div class="text-center">
          <div class="text-xs text-secondary">Lembur</div>
          <div class="fw-bold text-success">${fmtIdr(totalLembur)}</div>
        </div>
        <div class="text-center">
          <div class="text-xs text-secondary">Kasbon</div>
          <div class="fw-bold text-danger">-${fmtIdr(totalKasbon)}</div>
        </div>
        <div class="text-center">
          <div class="text-xs text-secondary">Total Bersih</div>
          <div class="fw-bold text-success">${fmtIdr(totalBersih)}</div>
        </div>
      </div>

      <!-- Per Proyek -->
      <div class="mb-16">
        <div class="text-xs fw-bold mb-8">Pendapatan per Proyek</div>
        ${Array.from(byProject.entries()).map(([prjName, data]) => `
          <div style="display:flex;justify-content:space-between;padding:6px 10px;margin-bottom:4px;background:var(--bg-input);border-radius:var(--radius);">
            <span class="text-sm">${esc(prjName)} (${data.count} hari)</span>
            <span class="fw-bold text-success">${fmtIdr(data.total)}</span>
          </div>
        `).join('')}
      </div>
      ...
      <!-- 30 Riwayat Terakhir -->
      <div>
        <div class="text-xs fw-bold mb-8">30 Riwayat Terakhir</div>
        <table class="data-table" style="font-size:0.8rem;">
          ...
          <tbody>
            ${logs.map((l, idx) => {
              const prj = state.projects.find(p => p.id === l.project_id);
              const acts = activities.filter(a => a.attendance_id === l.id);
              return `
                <tr>
                  <td>${fmtDate(l.created_at)}</td>
                  <td>${esc(prj?.name || '-')}</td>
                  <td>${l.status === 'verified' ? '<span class="badge badge-online">HADIR</span>' : l.status === 'absent' ? '<span class="badge badge-offline">TIDAK</span>' : '<span class="badge">PENDING</span>'}</td>
                  <td class="text-right">${fmtIdr((l.basic_salary||0) + (l.overtime_pay||0) - (l.cash_advance||0))}</td>
                </tr>
                ${acts.length > 0 ? `
                <tr style="background:var(--bg-hover);">
                  <td colspan="4" style="padding:4px 12px;">
                    <div class="text-xs text-secondary">Kegiatan: ${acts.map(a => esc(a.description)).join(', ')}</div>
                  </td>
                </tr>` : ''}
              `;
            }).join('')}
